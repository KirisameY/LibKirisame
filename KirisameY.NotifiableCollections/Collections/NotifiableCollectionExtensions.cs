...
/// <summary>
///     把任意 <see cref="IReadOnlyNotifiableCollection{T}"/> 包装成只读视图的扩展方法：
///     库自身的通知模型为 <c>AsReadOnlyNotifiableCollection</c>，
///     桥接到标准库通知模型的观测版本为 <c>AsReadOnlyObservableCollection</c>。
///     <br/>
///     Extension methods that wrap an arbitrary <see cref="IReadOnlyNotifiableCollection{T}"/> into a read-only
///     view: the library's own notification model is <c>AsReadOnlyNotifiableCollection</c>, and the observable
///     variant bridging to the standard one is <c>AsReadOnlyObservableCollection</c>.
/// </summary>
